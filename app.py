...
from PIL import Image  # noqa: F401 (kept for potential future usage)
...
DATASET_PATH = os.path.join('dataset', 'dogs')
MATCHES_FOLDER = os.path.join('static', 'matches')
...
# Prefer environment variables with fallbacks to provided defaults
db_config = {
    'host': os.getenv('DB_HOST', 'localhost'),
    'user': os.getenv('DB_USER', 'root'),
    'password': os.getenv('DB_PASSWORD', 'iniyan1842006'),
    'database': os.getenv('DB_NAME', 'pupaid'),
}
...
try:
    ...
    print('Spacy model not found. Attempting to download en_core_web_md...')
    try:
        os.system('python -m spacy download en_core_web_md')
        nlp = spacy.load('en_core_web_md')
    except Exception:
        nlp = None
        print('Failed to load spaCy model. Text diagnosis will be limited until model is installed.')
...
class SkinInfectionDetector:
    ...
    def train_model(self):
        ...
        if self.feature_extractor is None:
            print('Feature extractor unavailable. Training aborted.')
            return
    ...
    def predict_infection(self, test_image_path):
        if not self.model_trained or self.feature_extractor is None or self.train_features is None or len(self.train_features) == 0:
        ...
        try:
            test_feature = self.feature_extractor.predict(np.expand_dims(processed_img, axis=0), verbose=0)
        except Exception as e:
            return {'error': f'Failed to extract features from test image: {e}'}
    ...
    def create_result_visualization(self, test_img_path, label, confidence, output_path):
        try:
            ...
            print('Failed to load test image for visualization.')
            return
        ...
        contagious = disease_info['contagious'] if disease_info else 'Unknown'
        ...
        plt.text(0.05, 0.4, f'Description: {snippet}', fontsize=10)
        plt.text(0.05, 0.3, f'Severity: {severity}', fontsize=10)
        plt.text(0.05, 0.2, f'Contagious: {contagious}', fontsize=10)
        plt.axis('off')
...
def process_text_query(query):
    if nlp is None:
        return {'error': 'NLP model unavailable. Please install en_core_web_md.'}
    ...
    try:
        ...
        if connection:
            try:
                ...
                cursor.execute('SELECT id FROM pets WHERE user_id = %s', (session['user_id'],))
    ...
    try:
        ...
        if connection:
            try:
                ...
                if pet:
                    return jsonify({'success': True, 'dog_info': pet})
                else:
                    return jsonify({'success': False, 'error': 'No dog information found'}), 404
...
def analyze():
    try:
        ...
        if detector is None or not detector.model_trained:
...
def search():
    try:
        ...
        if connection:
            try:
                cursor = connection.cursor()
                cursor.execute('INSERT INTO searches (user_id, query) VALUES (%s, %s)', (session['user_id'], query))
                connection.commit()
                cursor.close()
                connection.close()
            except Error as e:
